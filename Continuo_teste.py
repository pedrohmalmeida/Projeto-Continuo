...
with c1:
    ...
    """)
    modalidade_associacao=st.selectbox('**Modalidade de associação:**',['Associado','Semi Associado','Não Associado'])
    st.markdown("""
...
with c1:
    ...
    Dil_min,Dil_max=st.slider('**Taxa de Diluição (1/h):**',min_value=0.00,max_value=Dcritico+0.01,value=(0.0,0.50*Dcritico))
    if modalidade_processo == 'Padrão':
    ...
    st.warning(f'Taxa de diluição Crítica (Dc): {Dcritico:.3f}')

    st.subheader('Visualização das Fórmulas')
    mostrar_Dcritico=st.checkbox('Mostrar Dcritico no gráfico',False)
    if st.checkbox('Substrato'):
        st.latex(r"""
...
with c2:
    ...
    ax1.set_title(f'{modalidade_associacao}')
    ax1.grid(True)
